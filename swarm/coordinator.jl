...
function get_task_status(task_id::String)::Union{Dict, Nothing}
    ...
    task = findfirst(t -> t.id == task_id, COORDINATOR_STATE.task_queue)
    if !isnothing(task)
        return Dict(
            "task" => COORDINATOR_STATE.task_queue[task],
            "status" => COORDINATOR_STATE.task_queue[task].status
        )
    end
...
export start_swarm_coordinator, stop_swarm_coordinator, submit_wallet_analysis_task,
       submit_token_analysis_task, submit_transaction_analysis_task, get_task_status,
       get_swarm_status, COORDINATOR_STATE
